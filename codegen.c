...
static void load(Type *ty) {
  ...
  if (ty->size == 1) {
    println("  mov%cbl (%%rax), %%eax", ty->is_unsigned ? 'z' : 's');
  } else if (ty->size == 2) {
    println("  mov%cwl (%%rax), %%eax", ty->is_unsigned ? 'z' : 's');
  } else if (ty->size == 4) {
...
static const char *cast_table[][11] = {
...
static void gen_expr(Node *node) {
  ...
  case ND_NUM: {
    ...
    case TY_LDOUBLE: {
      union { long double f80; uint64_t u64[2]; } u = { 0 };
      u.f80 = node->fval;
